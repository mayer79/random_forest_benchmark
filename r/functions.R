make_X <- function(n = 1000, m = 10, m_int = floor(m / 2), 
                   m_float = m - m_int, seed = 1) {
  set.seed(seed)
  ...
  X_float <- matrix(runif(n * m_float), ncol = m_float, 
                    dimnames = list(NULL, paste0("X", m_int + seq_len(m_float))))
  data.frame(X_int, X_float)
}

make_y <- function(X, objective = c("mse", "poisson"), strength = 5) {
  objective <- match.arg(objective)
  beta <- seq_len(ncol(X)) - 1
  mu <- data.matrix(X) %*% beta
